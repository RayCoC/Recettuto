        ...
        if (!empty($data['avis'])) {
            foreach ($data['avis'] as $item => $value) {
                ...
                echo '<h5><a href="index.php?action=profil&module=mod_Profil&login=' . $value['user'] . '" class="profile-link">' . $value['user'] . '</a></h5>
                                        <p>' . $value['message'] . '</p>
                                        <a class="btn text-green likeCommentaire" value=' . $value['idAvis'] . '><i class="fa fa-thumbs-up"></i></a>
                                        <a class="btn text-yellow signaler" id="signalement'.$value['idAvis'].'" value=' . $value['idAvis'] . '><i class="fas fa-exclamation-triangle"></i></a>
                                        <p id="nbLikeComment'.$value['idAvis'].'">' . $value['like'] . '</p>';
            }
            echo '</div>';
        }
    }
